...
export default function Overview() {
	...
	function getColor(data: number) {
		...
		criteriaDetailDatas.forEach((c_data: any) => {
			if (
				c_data.forecast_osf_criteria_id === parseInt(SelectValue)
			) {
				criteria = c_data;
			}
		});
								...
								<Stack
									...
									<FormControl
										...
										<Select
											...
											id='model-data'
											value={SelectValue}
											onChange={(s) => {if (s.target.value === 'No datas Available') { return }; setSelectValue(s.target.value)}}
											style={{
												height: 30,
												...
												<TableHead>
													<TableRow>
														{TableDatas.map((datas: any) =>
															datas.slice(0, 1).map((data: any, index: number) => (
																<TableCell
																	key={index}
																	style={{
																		padding: 0,
